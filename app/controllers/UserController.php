...
class UserController extends BaseController {
	...
	public function login(){
		...
		if(Auth::attempt($userdata)){
			...
			if($user->active == '0'){
				...
				return Redirect::to('login')->with('message', FlashMessage::DisplayAlert('Your account is currently disabled, Please contact your Administrator for additional details', 'danger'));
			...
			return Redirect::to('/')->with('message', FlashMessage::DisplayAlert('Login Successful', 'success'));
	...
	*/
	public function signup(){
		...
		// Once the record has been saved to the database, then redirect the user back to the login screen
	...
	*/
	public function resetpassword($resetcode){
		...
		// If the DB search comes back with records form the query
		if($user->count()){
			// Set the user variable to the first returned record
			$user = $user->first();

			// Set the user password to the value stored in password_temp, and then clear password_temp and resetcode.
			...
			if($user->save()){
				...
				return Redirect::to('login')->with('message', FlashMessage::DisplayAlert('Your account has been reset. you can now login with the password sent to your e-mail.', 'success'));
			}

		} // End user count

		// If no user record was found, then inform the user that the reset code was not found in the database
		return Redirect::to('login')->with('message', FlashMessage::DisplayAlert('Could not recover account. Please contact your Administrator for further assistence.', 'danger'));
